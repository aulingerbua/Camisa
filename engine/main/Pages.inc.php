...
class Pages extends Registry
{
    ...
    /**
     * updates also the parent names of the children of the changed page.
     *
     * {@inheritdoc}
     * @see Registry::update()
     */
    function update(array $input, $which)
    {
        $db = db_init();
        $log = start_system_log("update");
        if ($parent = $input['name']) {
            $oldpar = self::retrieve($input[$this->uniquefield], NULL, 'name')[0]['name'];
            $parentqry = "update " . $this->table . " set parent='$parent' where parent='$oldpar'";
        }
        if ($success = parent::update($input, $which)) {
            $success = parent::executeQuery($parentqry);
        }
        if (! $success)
            $log->write("error in page update: " . $db->error);
        return $success;
    }

    /**
     * sets parent names of the children of the deleted parent page to NULL
     *
     * {@inheritdoc}
     * @see Registry::delete()
     */
    function delete($which, $constraints = NULL)
    {
        $db = db_init();
        $log = start_system_log("delete");
        if ($oldpar = self::retrieve($which, NULL, 'name')[0]['name']) {
            $parentqry = "update " . $this->table . " set parent=NULL where parent='$oldpar'";
        }
        if ($success = parent::delete($which, $constraints = NULL)) {
            $success = parent::executeQuery($parentqry);
        }
        if (! $success)
            $log->write("error in page delete: " . $db->error);
        return $success;
    }

    /**
     * finds children of Pages
     *
     * @param string $parent
     * @return mixed
     */
