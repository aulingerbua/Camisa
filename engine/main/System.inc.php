...
<div class="box-inside-form">
	...
	<div>
		<label for="pwd">user name</label> <input type="text" name="username"
			value="<?=$iniVals['username']?>"> <label for="pwd">password</label>
		<input type="password" name="pwd"> <label for="html">html mail</label><input
			type="checkbox" name="html" <?=$html_checked?>><br> <label
			for="smtpauth">smtp authentification</label><input type="checkbox"
			name="smtpauth" <?=$smtpa_checked?>>
	</div>
</div>
<?php
		if ($iniVals) {
	...
	/**
	 * Saves the sendmail settings in the system table.
	 *
	 * @param array $data        	
	 * @return NULL|string[]|number[]|unknown[]
	 */
	public function saveSendmailSettings(array $data) {
		if (! $data)
			return NULL;
		$db = db_init ();
		$log = start_system_log ( "saveSendmailSettings" );
		...
		$settings [] = [ 
				...
				'entry' => 'fromemail',
				'version' => 1,
				'value' => $db->escape_string ( $data ['fromemail'] ),
				'grp' => 'sendmail',
				'info' => 'email address used for the from-header' 
		];
		$settings [] = [ 
				...
				'entry' => 'fromname',
				'version' => 1,
				'value' => $db->escape_string ( $data ['fromname'] ),
				'grp' => 'sendmail',
				'info' => 'name used for the from-header' 
		];
		$settings [] = [ 
				...
				'entry' => 'smtp',
				'version' => 1,
				'value' => $db->escape_string ( $data ['smtp'] ),
				'grp' => 'sendmail',
				'info' => 'the smtp server name' 
		];
		$settings [] = [ 
				...
				'entry' => 'port',
				'version' => 1,
				'value' => $db->escape_string ( $data ['port'] ),
				'grp' => 'sendmail',
				'info' => 'the smtp port to use' 
		];
		$settings [] = [ 
				...
				'entry' => 'username',
				'version' => 1,
				'value' => $db->escape_string ( $data ['username'] ),
				'grp' => 'sendmail',
				'info' => 'mail user name' 
		];
		if ($data ['pwd']) {
			$settings [] = [ 
					...
					'entry' => 'pwd',
					'version' => 1,
					'value' => $db->escape_string ( $data ['pwd'] ),
					'grp' => 'sendmail',
					'info' => 'mail user password' 
			];
		}
		...
		if ($data ['insert']) {
			foreach ( $settings as $row ) {
				$qry [] = "INSERT INTO system (" . implode ( ",", array_keys ( $row ) ) . ") VALUES ('" . implode ( "','", $row ) . "')";
			}
		} else if ($data ['update']) {
			foreach ( $settings as $row ) {
				$qry [] = "UPDATE system SET value='" . $row ['value'] . "' WHERE grp='sendmail' AND entry='" . $row ['entry'] . "'";
			}
		}
		try {
			self::executeQuery ( implode ( ";", $qry ), true );
		} catch ( Exception $e ) {
