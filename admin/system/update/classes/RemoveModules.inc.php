	...
	public static function backendNavEntry($entry = NULL) {
		...
		if (is_array ( $entry )) {
			foreach ( $entry as $ent ) {
				$k = array_search ( $ent, array_column ( $current_menu ['menueitem'], "entry" ) );
				if ($k !== FALSE)
					unset ( $current_menu ['menueitem'] [$k] );
			}
		} else {
			$k = array_search ( $entry, array_column ( $current_menu ['menueitem'], "entry" ) );
			if ($k !== FALSE)
				unset ( $current_menu ['menueitem'] [$k] );
		}
		if (! empty ( $current_menu ))
			$saved = file_put_contents ( BasePath . "backend/menueItems.json", json_encode ( $current_menu ) );
