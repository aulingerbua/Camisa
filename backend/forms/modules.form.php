...
if ($module = $_GET ['install']) {
	if (file_exists ( ModulesPath . "/$module/install.php" )) {
		...
		if (! $inst->isInstalled ()) {
			...
			if ($copyImages)
				if (! $inst->copySystemDirectory ( ModulesPath . "/$module/img", ImagesPath . "/" . $module )) {
					echo "<p class='alert'>Images could not be copied.</p>";
					die ();
				}
			if ($copyButtons)
				if (! $inst->copySystemDirectory ( ModulesPath . "/$module/buttons", ImagesPath . "/" . $module )) {
					echo "<p class='alert'>Images could not be copied.</p>";
					die ();
				}
	...
	if (file_exists ( ModulesPath . "/$module/update.php" )) {
		...
		$upd = new Install ( $module, $version );
		
		if ($functions)
			if (! $upd->installFunctions ( $functions )) {
				echo "<p class='alert'>Functions could not be updated.</p>";
				die ();
			}
		if ($AJAXscripts)
			if (! $upd->installAJAXscripts ( $AJAXscripts )) {
				echo "<p class='alert'>AJAXscripts could not be updated.</p>";
				die ();
			}
		if ($JavaScripts)
			if (! $upd->installJavaScripts ( $JavaScripts )) {
				echo "<p class='alert'>JavaScripts could not be updated.</p>";
				die ();
			}
		if ($Templates)
			if (! $upd->installTemplates ( $installTemplates )) {
				echo "<p class='alert'>Templates could not be updated.</p>";
				die ();
			}
		if ($Styles)
			if (! $upd->installStyles ()) {
				echo "<p class='alert'>Styles could not be updated.</p>";
				die ();
			}
		if ($BackendStyles)
			if (! $upd->installBackendStyles ()) {
				echo "<p class='alert'>Backend styles could not be updated.</p>";
				die ();
			}
		if ($BackendNavEntry)
			if (! $upd->installBackendNavEntry ()) {
				echo "<p class='alert'>Backend navi entries could not be updated.</p>";
				die ();
			}
		if ($BackendForm)
			if (! $upd->installBackendForms ( $install_backend_form )) {
				echo "<p class='alert'>Backend forms could not be updated.</p>";
				die ();
			}
		if ($copyImages)
			if (! $upd->copySystemDirectory ( ModulesPath . "/$module/img", ImagesPath . "/" . $module )) {
				echo "<p class='alert'>Images could not be copied.</p>";
				die ();
			}
		if ($copyButtons)
			if (! $upd->copySystemDirectory ( ModulesPath . "/$module/buttons", ImagesPath . "/" . $module )) {
				echo "<p class='alert'>Images could not be copied.</p>";
				die ();
			}
		$upd->updateSystemTable ();
	} else
